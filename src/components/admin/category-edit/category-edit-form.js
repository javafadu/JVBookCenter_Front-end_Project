...
const CategoryEditForm = () => {
  ...
  const loadData = async () => {
    ...
    try {
      const resp = await getCategoryWithId(categoryId);
    ...
    <Form noValidate onSubmit={formik.handleSubmit} className="px-2">
...

      {
        <Alert variant="danger" className="mt-5">
          {access ? "" : `Only administrator can update category details `}
          {initialValues.builtIn
            ? ` Built-in categories can not be deleted and updated. If you want to update please change built-in status firstly`
            : ``}
        </Alert>
      }
