...
import { mostPopularBooksTable } from "../../../api/report-service";
...
const MostPopularBooksTable = () => {
  ...
  const loadData = async (amount = 50, page = 0, perpage = perPage) => {
    try {
      setLoading(true);
      const resp = await mostPopularBooksTable(amount, page, perPage);
      const {
        content,
        numberOfElements,
        size,
...
export default MostPopularBooksTable;
