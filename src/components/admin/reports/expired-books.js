...
import { expiredBooksTable } from "../../../api/report-service";
...
const ExpiredBooksTable = () => {
  ...
  const loadData = async (page = 0, perpage = perPage) => {
    try {
      setLoading(true);
      const resp = await expiredBooksTable(page, perPage);
      const {
        content,
        numberOfElements,
        size,
...
export default ExpiredBooksTable;
